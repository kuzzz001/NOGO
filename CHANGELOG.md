...
#### Issue 1: gameStage 分支顺序修正
- **问题**：`if (gameStage == 0) ... else if (gameStage == 2) ... else ...` 顺序为 0→2→1，违反自然语义
- **修复**：改为 0→1→2 顺序：开局→中盘→残局，与 `gameStage` 定义值一致
- **影响**：功能行为不变（gameStage 只取 0/1/2），纯可读性改进

#### Issue 2: evalMovePriority 对手残气评分同步
- **问题**：`evalMovePriority()` 中对邻格对手残气评分为 `1气+60 / 2气+25 / 3气+8`，与 `simulate()` 中的 `oppPressure` 评分 `1气+80 / 2气+30 / 3气+10` 不一致，也与 CHANGELOG 记录不符
- **修复**：`evalMovePriority()` 统一为 `80 / 30 / 10`，与 `simulate()` 和 CHANGELOG 保持一致
- **影响**：MCTS 扩张阶段的排序更重视封锁对手，搜索引导更准确

---

## [v1.1.0] - 2026-05-12

### P0-P3 全面战略重构：从"活得久"转向"让对手先死"

#### P0 — 核心战术修正（2 项）

##### 1. 群体大小惩罚（连通块越大扣分越多）
- **countLiberties** 新增 `outGroupSize` 输出参数，在 DFS 中同时统计连通块大小
- **simulate** 评估公式新增 `groupPenalty = groupSize² × 3`：5 子连通块扣 75 分，10 子扣 300 分
- **核心理念**：NoGo 中大龙 = 致命弱点，所有棋子共享有限的气

##### 2. simulate 步数动态化
- 旧：固定 25 步后返回平局（早期 rollout 永远无法到达终局）
- 新：`maxSteps = min(emptySpaces, 45)`：20 格以下跑满终局，45 格以上也能覆盖大半对局
- **效果**：残局 rollout 能精准判定胜负，不再返回无意义的平局

#### P1 — 进攻性策略（2 项）

##### 3. 开局书改为跳格式
- 旧：`{4,4}, {3,4}, {4,3}, {5,4}, {4,5}` — 5 子十字连通块
- 新：`{3,3}, {5,5}, {3,5}, {5,3}, {2,2}, {6,6}, {2,6}, {6,2}` — 对角线跳格，每子间距 ≥2 格
- **效果**：前 8 手不形成任何连通块，最大化空间利用率

##### 4. 对手残气检测（oppPressure）
- 落子后检查 4 邻格对手棋子气数：
  - `1气 → +80`（几乎锁死）
  - `2气 → +30`（高压）
  - `3气 → +10`（施压）
- **核心理念**：主动压缩对手生存空间是 NoGo 最强战术

#### P2 — 评估智能化（2 项）

##### 5. 局面阶段自适应权重
- 开局 (>45空)：`10 - max(|x-4|, |y-4|)` 中心导向
- 中盘 (20-45空)：`positionWeight[x][y]` 原有权重矩阵
- 残局 (<20空)：边缘惩罚 `max(0, weight - 3)`
- **核心理念**：不同阶段的空间价值不同，残局边缘=陷阱

##### 6. MCTS 扩张阶段用评估引导（Prior Knowledge）
- 新增 `evalMovePriority()`：综合评估落子的战术价值
- 评分公式：`lib×4 - groupSize²×3 + oppLib压迫 + blockedSides×封锁 + 位置权重 + 历史表/8`
- 扩张子节点时按战术优先级排序，替代纯历史表排序

#### P3 — 搜索优化（1 项）

##### 7. 封锁优先排序替代纯历史表
- 新增 `sortMovesByPriority()`：按 `evalMovePriority()` 排序
- MCTS 根节点和扩张节点的 children 均按战术优先级排列
- 历史表权重降级为辅助参考（`/8`），战术评估为主导

#### 技术总结

| 维度 | v1.0.0 (旧) | v1.1.0 (新) |
|------|------------|------------|
| simulate 步数 | 固定 25 | 动态 min(空位, 45) |
| 评估维度 | lib, blocked, pos | +groupPenalty, +oppPressure, +stageWeight |
| 得分方向 | 自身存活 | 封锁对手 + 分散自身 |
| 开局策略 | 中心十字连通 | 对角线跳格分散 |
| 扩张排序 | 历史表 | 战术优先级 + 历史辅助 |
| countLiberties | 气数 | 气数 + 群体大小 |
...
#### 修复
- **evaluateThreats 添加缺失的气数处理**：`liberties == 2` 和 `liberties == 0` 之前无处理，导致评估盲区
  - `liberties >= 4` → `+5`（安全）
  - `liberties == 3` → `+2`（较安全）
  - `liberties == 2` → `-2`（偏危险，对手一夹只剩 1 气）
  - `liberties == 1` → `-5`（危险）
  - `liberties == 0` → `-20`（不应出现，防御性扣分）
